...
echo ECS_CLUSTER=GeeksAcademy >> /etc/ecs/ecs.config
...
sed -i 's/Hostname=.*/Hostname=GeeksAcademy/' /etc/zabbix/zabbix_agentd.conf
curl http://169.254.169.254/latest/meta-data/public-ipv4 | xargs -I % sh -c "sed -i 's/# HostInterface=.*/HostInterface=%/' /etc/zabbix/zabbix_agentd.conf" %
